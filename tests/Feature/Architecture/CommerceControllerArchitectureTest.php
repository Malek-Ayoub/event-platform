...
class CommerceControllerArchitectureTest extends TestCase
{
    /** @var list<class-string> */
    private array $commerceControllers = [
    ...
    #[Test]
    public function commerce_controllers_extend_base_api_controller(): void
    {
        foreach ($this->commerceControllers as $controller) {
            $this->assertTrue(
    ...
    #[Test]
    public function commerce_controllers_do_not_import_domain_models(): void
    {
        foreach ($this->commerceControllers as $controller) {
    ...
    #[Test]
    public function commerce_controllers_do_not_use_forbidden_patterns(): void
    {
        foreach ($this->commerceControllers as $controller) {
            $source = file_get_contents((new ReflectionClass($controller))->getFileName());
            $this->assertIsString($source);
    ...
    #[Test]
    public function commerce_controllers_inject_exactly_one_domain_service(): void
    {
        foreach ($this->commerceControllers as $controller) {
    ...
    #[Test]
    public function commerce_controller_actions_use_form_requests(): void
    {
        foreach ($this->commerceControllers as $controller) {
    ...
    #[Test]
    public function commerce_controllers_use_api_resources_and_response_helpers(): void
    {
        foreach ($this->commerceControllers as $controller) {
            $source = file_get_contents((new ReflectionClass($controller))->getFileName());
            $this->assertIsString($source);
