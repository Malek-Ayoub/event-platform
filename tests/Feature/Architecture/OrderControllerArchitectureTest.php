...
class OrderControllerArchitectureTest extends TestCase
{
    /** @var list<class-string> */
    private array $orderControllers = [
    ...
    #[Test]
    public function order_controllers_extend_base_api_controller(): void
    {
        foreach ($this->orderControllers as $controller) {
            $this->assertTrue(
    ...
    #[Test]
    public function order_controllers_do_not_import_domain_models(): void
    {
        foreach ($this->orderControllers as $controller) {
    ...
    #[Test]
    public function order_controllers_do_not_use_forbidden_patterns(): void
    {
        foreach ($this->orderControllers as $controller) {
            $source = file_get_contents((new ReflectionClass($controller))->getFileName());
            $this->assertIsString($source);
    ...
    #[Test]
    public function order_controllers_inject_exactly_one_domain_service(): void
    {
        foreach ($this->orderControllers as $controller) {
    ...
    #[Test]
    public function order_controller_actions_use_form_requests(): void
    {
        foreach ($this->orderControllers as $controller) {
    ...
    #[Test]
    public function order_controllers_use_api_resources_and_response_helpers(): void
    {
        foreach ($this->orderControllers as $controller) {
            $source = file_get_contents((new ReflectionClass($controller))->getFileName());
            $this->assertIsString($source);
